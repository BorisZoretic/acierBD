...
class FastechPayements extends FastechModel {
	...
	public function getPayementFromDB($id_employe, $id_work_week) {
		...
		$sql = "SELECT id_payement FROM `" . $this->table_name . "`  WHERE id_employe = '" . $id_employe  . "' AND id_work_week = '" . $id_work_week . "'";
	...
	function getListOfActiveBDObjectsWithWeekId($weekId){
		include $_SERVER ["DOCUMENT_ROOT"] . '/database_connect.php';
		
		$internalAttributes = get_object_vars ( $this );
		
		$sql = "SELECT * FROM `" . $this->table_name . "` WHERE id_state = 1 AND id_work_week = '" . $weekId . " '";
	...
	function verfyPayementForPrimeWorkWeek($weekId,$prime){
		include $_SERVER ["DOCUMENT_ROOT"] . '/database_connect.php';
		$sql = "SELECT * 
		FROM payements p join prime_payement pp on p.id_payement = pp.id_payement
		WHERE p.id_work_week = ". $weekId . " AND pp.prime = '". $prime ."'
		";
		$result = $conn->query ( $sql );
	...
	function getListOfActiveBDObjectsWithWeekIdCCQ($weekId, $isCCq){
		include $_SERVER ["DOCUMENT_ROOT"] . '/database_connect.php';
		
		$internalAttributes = get_object_vars ( $this );
		
		$sql = "SELECT * FROM `" . $this->table_name . "` as p JOIN employees e on p.id_employe = e.order WHERE p.id_state = 1  AND e.bool_ccq = ".$isCCq." AND p.id_work_week = '" . $weekId . " '";
		if($this->primary_key =="order"){
			$sql .= " order by `order`";
		}
		//echo $sql;
...
$payement->getListOfActiveBDObjectsWithWeekIdCCQ(4, 1);*/
?>